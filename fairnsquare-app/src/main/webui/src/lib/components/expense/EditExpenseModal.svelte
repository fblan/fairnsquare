<script lang="ts">
  /**
   * EditExpenseModal - Story FNS-002-6
   * Modal for editing existing expenses with dirty tracking and delete capability
   */

  import { updateExpense, deleteExpense, type Expense, type Participant, type SplitMode } from '$lib/api/splits';
  ...
  // Dirty tracking - compare against original expense values
  ...
  $effect(() => {
    if (open && expense) {
      ...
      setTimeout(() => {
        document.getElementById('expense-amount-edit-modal')?.focus();
      }, 50);
    }
  });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      addToast({
        type: 'error',
        message: error.detail || 'Failed to update expense. Please try again.',
      });
    } finally {
      isLoading = false;
    }
  ...
  >
    ...
    >
      ...
      <div class="flex items-center justify-between p-4 border-b">
        <h2 id="edit-expense-title" class="text-lg font-semibold">
          Edit Expense
        </h2>
        <Button
          variant="ghost"
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="p-4 space-y-4">
        ...
        <div class="space-y-2">
          <Label for="expense-amount-edit-modal">Amount (€)</Label>
          <Input
            id="expense-amount-edit-modal"
            type="number"
            step="0.01"
            min="0.01"
            placeholder="€0.00"
        ...
        <div class="space-y-2">
          <Label for="expense-description-edit-modal">Description (optional)</Label>
          <Input
            id="expense-description-edit-modal"
            type="text"
        ...
        <!-- Payer Select -->
        <div class="space-y-2">
          <Label for="expense-payer-edit-modal">Payer</Label>
          <Select.Root
          ...
          >
            <Select.Trigger id="expense-payer-edit-modal" class="min-h-[44px]" disabled={isLoading || isDeleting}>
        ...
        <div class="space-y-2">
          ...
          <RadioGroup.Root bind:value={splitMode} class="flex flex-col space-y-2" disabled={isLoading || isDeleting}>
            <div class="flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent min-h-[44px]">
              <RadioGroup.Item value="BY_NIGHT" id="edit-modal-mode-by-night" />
              <Label for="edit-modal-mode-by-night" class="flex items-center gap-2 cursor-pointer flex-1">
                <Moon class="h-4 w-4" aria-hidden="true" />
                <span>By Night</span>
              </Label>
            </div>
            <div class="flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent min-h-[44px]">
              <RadioGroup.Item value="EQUAL" id="edit-modal-mode-equal" />
              <Label for="edit-modal-mode-equal" class="flex items-center gap-2 cursor-pointer flex-1">
        ...
        <!-- Delete Expense Button (below form, before action buttons) -->
        ...
        <div class="flex gap-2 pt-2">
          <Button
            type="submit"
            disabled={!isDirty || !isValid || isLoading || isDeleting}
